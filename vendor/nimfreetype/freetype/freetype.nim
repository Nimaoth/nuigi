...
const
  FT_ENCODING_NONE*      = ((0.uint32 shl 24) or (0.uint32 shl 16) or (0.uint32 shl 8) or 0.uint32).FT_Encoding
  FT_ENCODING_MS_SYMBOL* = (('s'.uint32 shl 24) or ('y'.uint32 shl 16) or ('m'.uint32 shl 8) or 'b'.uint32).FT_Encoding
  FT_ENCODING_UNICODE*   = (('u'.uint32 shl 24) or ('n'.uint32 shl 16) or ('i'.uint32 shl 8) or 'c'.uint32).FT_Encoding

  FT_ENCODING_SJIS*      = (('s'.uint32 shl 24) or ('j'.uint32 shl 16) or ('i'.uint32 shl 8) or 's'.uint32).FT_Encoding
  FT_ENCODING_GB2312*    = (('g'.uint32 shl 24) or ('b'.uint32 shl 16) or (' '.uint32 shl 8) or ' '.uint32).FT_Encoding
  FT_ENCODING_BIG5*      = (('b'.uint32 shl 24) or ('i'.uint32 shl 16) or ('g'.uint32 shl 8) or '5'.uint32).FT_Encoding
  FT_ENCODING_WANSUNG*   = (('w'.uint32 shl 24) or ('a'.uint32 shl 16) or ('n'.uint32 shl 8) or 's'.uint32).FT_Encoding
  FT_ENCODING_JOHAB*     = (('j'.uint32 shl 24) or ('o'.uint32 shl 16) or ('h'.uint32 shl 8) or 'a'.uint32).FT_Encoding
  ...
  FT_ENCODING_ADOBE_STANDARD* = (('A'.uint32 shl 24) or ('D'.uint32 shl 16) or ('O'.uint32 shl 8) or 'B'.uint32).FT_Encoding
  FT_ENCODING_ADOBE_EXPERT*   = (('A'.uint32 shl 24) or ('D'.uint32 shl 16) or ('B'.uint32 shl 8) or 'E'.uint32).FT_Encoding
  FT_ENCODING_ADOBE_CUSTOM*   = (('A'.uint32 shl 24) or ('D'.uint32 shl 16) or ('B'.uint32 shl 8) or 'C'.uint32).FT_Encoding
  FT_ENCODING_ADOBE_LATIN_1*  = (('l'.uint32 shl 24) or ('a'.uint32 shl 16) or ('t'.uint32 shl 8) or '1'.uint32).FT_Encoding

  FT_ENCODING_OLD_LATIN_2*    = (('l'.uint32 shl 24) or ('a'.uint32 shl 16) or ('t'.uint32 shl 8) or '2'.uint32).FT_Encoding
  FT_ENCODING_APPLE_ROMAN*    = (('a'.uint32 shl 24) or ('r'.uint32 shl 16) or ('m'.uint32 shl 8) or 'n'.uint32).FT_Encoding
...
func hasColor*(face: FT_Face): bool =
...
proc FT_Init_FreeType*(alibrary: var FT_Library): FT_Error {.ftimport.}
proc FT_Done_FreeType*(library: FT_Library): FT_Error {.ftimport.}
...
proc FT_New_Memory_Face*(library: FT_Library; file_base: cstring;
  file_size: FT_Long; face_index: FT_Long; aface: var FT_Face): FT_Error {.ftimport.}

proc FT_Open_Face*(library: FT_Library; args: var FT_Open_Args; face_index: FT_Long;
  aface: var FT_Face): FT_Error {.ftimport.}

proc FT_Attach_File*(face: FT_Face; filepathname: cstring): FT_Error {.ftimport.}
proc FT_Attach_Stream*(face: FT_Face; parameters: var FT_Open_Args): FT_Error {.ftimport.}
proc FT_Reference_Face*(face: FT_Face): FT_Error {.ftimport.}
proc FT_Done_Face*(face: FT_Face): FT_Error {.ftimport.}
proc FT_Select_Size*(face: FT_Face; strike_index: FT_Int): FT_Error {.ftimport.}

proc newFace*(lib: FT_Library, filePathName: var string; faceIndex: int; face: var FT_Face): FT_Error {.inline.} =
  FT_New_Face(lib, filePathName.toCString, FT_Long(faceIndex), face)
...
proc FT_Request_Size*(face: FT_Face; req: var FT_Size_Request): FT_Error {.ftimport.}

proc FT_Set_Char_Size*(face: FT_Face; char_width, char_height: FT_F26Dot6;
  horz_resolution, vert_resolution: FT_UInt): FT_Error {.ftimport.}

proc FT_Set_Pixel_Sizes*(face: FT_Face; pixel_width: FT_UInt; pixel_height: FT_UInt): FT_Error {.ftimport.}
proc FT_Load_Glyph*(face: FT_Face; glyph_index: FT_UInt; load_flags: FT_Int32): FT_Error {.ftimport.}
proc FT_Load_Char*(face: FT_Face; char_code: FT_ULong; load_flags: FT_Int32): FT_Error {.ftimport.}
...
# template FT_LOAD_TARGET*[T](x: T): FT_Int32 =
...
const
  ...
  FT_LOAD_TARGET_NORMAL*  = (FT_RENDER_MODE_NORMAL.int32 and 15) shl 16
  FT_LOAD_TARGET_LIGHT*   = (FT_RENDER_MODE_LIGHT.int32 and 15) shl 16
  FT_LOAD_TARGET_MONO*    = (FT_RENDER_MODE_MONO.int32 and 15) shl 16
  FT_LOAD_TARGET_LCD*     = (FT_RENDER_MODE_LCD.int32 and 15) shl 16
  FT_LOAD_TARGET_LCD_V*   = (FT_RENDER_MODE_LCD_V.int32 and 15) shl 16


# template FT_LOAD_TARGET_MODE*(x: untyped): untyped =
#   ((FT_Render_Mode)(((x) shr 16) and 15))

proc FT_Set_Transform*(face: FT_Face; matrix: nil ptr FT_Matrix; delta: var FT_Vector) {.ftimport.}

const
  ft_render_mode_normal* = FT_RENDER_MODE_NORMAL
  ft_render_mode_mono*   = FT_RENDER_MODE_MONO

proc FT_Render_Glyph*(slot: FT_GlyphSlot; render_mode: FT_Render_Mode): FT_Error {.ftimport.}
...
proc FT_Get_Kerning*(face: FT_Face; left_glyph: FT_UInt; right_glyph: FT_UInt;
  kern_mode: FT_UInt; akerning: var FT_Vector): FT_Error {.ftimport.}

proc FT_Get_Track_Kerning*(face: FT_Face; point_size: FT_Fixed; degree: FT_Int;
  akerning: var FT_Fixed): FT_Error {.ftimport.}

proc FT_Get_Glyph_Name*(face: FT_Face; glyph_index: FT_UInt; buffer: FT_Pointer;
  buffer_max: FT_UInt): FT_Error {.ftimport.}

proc FT_Get_Postscript_Name*(face: FT_Face): cstring {.ftimport.}
proc FT_Select_Charmap*(face: FT_Face; encoding: FT_Encoding): FT_Error {.ftimport.}
proc FT_Set_Charmap*(face: FT_Face; charmap: FT_CharMap): FT_Error {.ftimport.}
proc FT_Get_Charmap_Index*(charmap: FT_CharMap): FT_Int {.ftimport.}
proc FT_Get_Char_Index*(face: FT_Face; charcode: FT_ULong): FT_UInt {.ftimport.}
proc FT_Get_First_Char*(face: FT_Face; agindex: var FT_UInt): FT_ULong {.ftimport.}
proc FT_Get_Next_Char*(face: FT_Face; char_code: FT_ULong; agindex: var FT_UInt): FT_ULong {.ftimport.}
proc FT_Get_Name_Index*(face: FT_Face; glyph_name: var FT_String): FT_UInt {.ftimport.}
...
proc FT_Get_SubGlyph_Info*(glyph: FT_GlyphSlot; sub_index: FT_UInt;
                          ...
                          p_transform: var FT_Matrix): FT_Error {.ftimport.}
const
...
proc FT_Get_FSType_Flags*(face: FT_Face): FT_UShort {.ftimport.}

proc FT_Face_GetCharVariantIndex*(face: FT_Face; charcode: FT_ULong;
  variantSelector: FT_ULong): FT_UInt {.ftimport.}

proc FT_Face_GetCharVariantIsDefault*(face: FT_Face; charcode: FT_ULong;
  variantSelector: FT_ULong): FT_Int {.ftimport.}

proc FT_Face_GetVariantSelectors*(face: FT_Face): ptr FT_Uint32 {.ftimport.}
proc FT_Face_GetVariantsOfChar*(face: FT_Face; charcode: FT_ULong): ptr FT_Uint32 {.ftimport.}
proc FT_Face_GetCharsOfVariant*(face: FT_Face; variantSelector: FT_ULong): ptr FT_Uint32 {.ftimport.}
proc FT_MulDiv*(a: FT_Long; b: FT_Long; c: FT_Long): FT_Long {.ftimport.}
proc FT_MulFix*(a: FT_Long; b: FT_Long): FT_Long {.ftimport.}
proc FT_DivFix*(a: FT_Long; b: FT_Long): FT_Long {.ftimport.}
proc FT_RoundFix*(a: FT_Fixed): FT_Fixed {.ftimport.}
proc FT_CeilFix*(a: FT_Fixed): FT_Fixed {.ftimport.}
proc FT_FloorFix*(a: FT_Fixed): FT_Fixed {.ftimport.}
proc FT_Vector_Transform*(vec: var FT_Vector; matrix: var FT_Matrix) {.ftimport.}
...
proc FT_Library_Version*(library: FT_Library; amajor: var FT_Int; aminor: var FT_Int;
  apatch: var FT_Int) {.ftimport.}

proc FT_Face_CheckTrueTypePatents*(face: FT_Face): FT_Bool {.ftimport.}

proc FT_Face_SetUnpatentedHinting*(face: FT_Face; value: FT_Bool): FT_Bool {.ftimport.}
