...
## - `!& proc <#!&,Hash,int64>`_ used to start or mix a hash value, and
...
proc `!$`*(h: Hash): Hash {.inline.} =
  ...
  ##
  ## This is only needed if you need to implement a `hash` proc for a new datatype.
  let h = cast[uint64](h) # Hash is practically unsigned.
  var res = h + h shl 3
...
proc hashData*(data: pointer, size: int64): Hash =
...
when defined(js):
  var objectID = 0
  proc getObjectId(x: pointer): int64 =
    {.emit: """
      if (typeof `x` == "object") {
        if ("_NimID" in `x`)
          `result` = `x`["_NimID"];
...
when defined(nimPreviewHashRef) or defined(nimdoc):
  proc hash*[T](x: ref[T]): Hash {.inline.} =
    ...
    runnableExamples("-d:nimPreviewHashRef"):
      ...
      assert A[int64](x: 3, y: 4).hash == A[int64](x: 3, y: 5).hash
...
proc rotl32(x: uint32, r: int64): uint32 {.inline.} =
...
proc hashVmImpl(x: cstring, sPos, ePos: int64): Hash =
  raiseAssert "implementation override in compiler/vmops.nim"

proc hashVmImpl(x: string, sPos, ePos: int64): Hash =
  raiseAssert "implementation override in compiler/vmops.nim"

proc hashVmImplChar(x: openArray[char], sPos, ePos: int64): Hash =
  raiseAssert "implementation override in compiler/vmops.nim"

proc hashVmImplByte(x: openArray[byte], sPos, ePos: int64): Hash =
  raiseAssert "implementation override in compiler/vmops.nim"
...
proc hashFarm(s: openArray[byte]): uint64 {.inline.} =
  ...
  var
    o: uint64 = 0         # s[] ptr arith -> variable origin variable `o`
    x = seed
    y = seed*k1 + 113
  ...
  while true:
    x = rotR(x + y + v[0] + load8(s, o.int+8), 37)*k1
    y = rotR(y + v[1] + load8(s, o.int+48), 42)*k1
    x = x xor w[1]
    y += v[0] + load8(s, o.int+40)
    z = rotR(z + w[0], 33)*k1
    v = weakLen32withSeeds(s, o.int+0 , v[1]*k1, x + w[0])
    w = weakLen32withSeeds(s, o.int+32, z + w[1], y + load8(s, o.int+16))
    swap z, x
    inc o, 64
    if o.int == eos: break
  ...
  v[0] += w[0]
  w[0] += v[0]
  x = rotR(x + y + v[0] + load8(s, o.int+8), 37)*mul
  y = rotR(y + v[1] + load8(s, o.int+48), 42)*mul
  x = x xor w[1]*9
  y += v[0]*9 + load8(s, o.int+40)
  z = rotR(z + w[0], 33)*mul
  v = weakLen32withSeeds(s, o.int+0 , v[1]*mul, x + w[0])
  w = weakLen32withSeeds(s, o.int+32, z + w[1], y + load8(s, o.int+16))
  swap z, x
...
proc hash*(sBuf: string, sPos, ePos: int64): Hash =
  ...
  maybeFailJS_Number()
  when not sHash2:
    result = cast[Hash](hashFarm(toOpenArrayByte(sBuf, sPos.int, ePos.int)))
  else:
    murmurHash(toOpenArrayByte(sBuf, sPos.int, ePos.int))
...
proc hashIgnoreStyle*(sBuf: string, sPos, ePos: int64): Hash =
  ## Efficient hashing of a string buffer, from starting
...
proc hashIgnoreCase*(sBuf: string, sPos, ePos: int64): Hash =
  ## Efficient hashing of a string buffer, from starting
...
proc hash*[A](aBuf: openArray[A], sPos, ePos: int64): Hash =
