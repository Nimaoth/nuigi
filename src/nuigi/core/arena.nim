...
proc alloc*(arena: var Arena, size: int, alignment: int = 0): pointer =
  ## Allocate memory on top of the stack.
  let alignment = if alignment == 0: size else: alignment
...
proc allocArray*[T](arena: var Arena, num: int, x: typedesc[T], align: int): ArrayView[T] =
  assert num >= 0
  if num == 0:
    return ArrayView[T].default
  let data = cast[ptr UncheckedArray[T]](arena.alloc(num * sizeof(T), align))
  return initArrayView(data, len = num, capacity = num)
...
proc realloc*(arena: var Arena, address: pointer, oldSize: int, size: int, alignment: int): pointer =
  ...
  for i in countdown(arena.buckets.high, 0):
    ...
    if addressInt >= cast[int](arena.buckets[i].data) and addressInt < cast[int](arena.buckets[i].data) + arena.buckets[i].len:
      let bucket = arena.buckets[i].addr
  ...
  assert false, "Trying to realloc address " & $addressInt & " in arena but it wasn't allocated with the arena"

proc resize*[T](arena: var Arena, arr: var ArrayView[T], newLen: int) =
  ## Resize ArrayView backing storage to `newLen` elements.
  ## Existing elements are preserved up to min(oldLen, newLen).
  assert newLen >= 0

  let oldCap = arr.cap
  let oldLen = arr.len
  let keptLen = min(oldLen, newLen)

  if newLen == 0:
    arr = default(ArrayView[T])
    return
  ...
  var newData: ptr UncheckedArray[T]
  if oldCap == 0:
    newData = cast[ptr UncheckedArray[T]](arena.alloc(newLen * sizeof(T), alignof(T)))
  else:
    newData = cast[ptr UncheckedArray[T]](
      arena.realloc(arr.data, oldCap * sizeof(T), newLen * sizeof(T), alignof(T))
    )

  arr = initArrayView(newData, len = keptLen, capacity = newLen)

proc resizeInline*[T](arena: var Arena, arr: var ArrayView[T], newLen: int): bool =
  let oldData = arr.data
  arena.resize(arr, newLen)
  if oldData != nil and arr.data != nil:
    return oldData == arr.data
  return true

proc bucketCount*(arena: Arena): int =
  arena.buckets.len

proc usedBytes*(arena: Arena): int =
  result = 0
  for i in 0 ..< arena.buckets.len:
    result += arena.buckets[i].len

proc capacityBytes*(arena: Arena): int =
  result = 0
  for i in 0 ..< arena.buckets.len:
    result += arena.buckets[i].capacity
