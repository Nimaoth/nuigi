...
when defined(nimony):
  import std/syncio
from freetype/freetype import FT_Library
...
proc requireWithinWidth(arrangement: UiTextArrangement, maxWidth: float32) =
  assert arrangement.size.x <= maxWidth + 0.01'f32

proc main() =
  var renderer = FontRender(fontLibrary: FT_Library(nil))
  assert renderer.init(flags = {})
  ...
  assert unlimited.lineCount == 1
  assert wrapped.lineCount == 2
  assert wrapped.size.y > unlimited.size.y
  ...
  assert separatorOverflow.lineCount == 2
  ...
  assert hardWrapped.lineCount == 3
